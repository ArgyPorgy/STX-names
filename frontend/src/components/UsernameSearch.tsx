...
export const UsernameSearch: React.FC = () => {
  ...
  // Fetch registration fee on mount (only once)
  useEffect(() => {
    let isMounted = true;
    getRegistrationFee()
      .then((fee) => {
        if (isMounted) setRegistrationFee(fee);
      })
      .catch(() => {
  ...
  useEffect(() => {
    ...
    if (address) {
      ...
      setTimeout(() => {
        if (isMounted) {
          getAddressUsername(address)
            .then((username) => {
              if (isMounted) setUserCurrentUsername(username);
            })
            .catch(() => {
              // Silently handle errors
            });
      ...
      if (isMounted) setUserCurrentUsername(null);
    }
    return () => {
      isMounted = false;
    };
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      try {
        ...
        // Silently handle errors - don't show error state on rate limit
        setStatus('idle');
      }
    }, 800); // Increased debounce delay from 300ms to 800ms

    return () => clearTimeout(timer);
  }, [username]); // Only depend on username, not function references
